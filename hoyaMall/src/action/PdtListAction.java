...
public class PdtListAction implements action.Action {
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		pdtPotList = pdtListSvc.getPdtPotList(where, orderby, cpage, psize);
		...
		pdtRankedList = pdtListSvc.getPdtRankList();
		// 현 페이지에서 보여줄 검색된 상품목록
		// 검색조건, 정렬조건, limit에서 사용할 값을 구하기 위해 현재페이지와 페이지크기를 인수로 가져감
		System.out.println("랭킹액션");
		request.setAttribute("pdtRankedList", pdtRankedList);
		// 상품목록 화면(pdt_list.jsp)으로 목록(pdtList)과 페이징 정보(pageInfo), 분류들을 request에 담아 가
		...
		request.setAttribute("pdtPotList", pdtPotList);
